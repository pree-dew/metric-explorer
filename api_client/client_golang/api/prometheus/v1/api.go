...
const (
	...
	epTSDBWithMetric  = apiPrefix + "/status/tsdb"
	epTopQueries      = apiPrefix + "/status/top_queries"
...
type API interface {
	...
	Query(ctx context.Context, query string, r time.Time, opts ...Option) (model.Value, Warnings, error)
	...
	// Top Queries
	TopQueries(ctx context.Context, topN, topNMaxLifeTime string) (TopQueriesResult, error)
	// TSDB returns the cardinality statistics.
	TSDB(ctx context.Context, topN, date string) (TSDBResult, error)
	// TSDBWithMetric return stats specific to the metric
	TSDBWithMetric(ctx context.Context, metric, focusLabel, topN, date string) (TSDBWithMetricResult, error)
...
// TopQueriesResult contains the result from querying the top queries endpoint.
type TopQueriesResult struct {
	TopN                 interface{}              `json:"topN"`
	MaxLifeTime          interface{}              `json:"maxLifetime"`
	LastQueriesCount     interface{}              `json:"search.queryStats.lastQueriesCount"`
	MinQueryDuration     interface{}              `json:"search.queryStats.minQueryDuration"`
	TopByCount           []map[string]interface{} `json:"topByCount"`
	TopByAverageDuration []map[string]interface{} `json:"topByAvgDuration"`
	TopBySumDuration     []map[string]interface{} `json:"topBySumDuration"`
}

type TSDBWithMetricResult struct {
	SeriesCountByMetricName      []Stat `json:"seriesCountByMetricName"`
	LabelValueCountByLabelName   []Stat `json:"labelValueCountByLabelName"`
	SeriesCountByFocusLabelValue []Stat `json:"seriesCountByFocusLabelValue"`
}
...
func (h *httpAPI) TSDBWithMetric(ctx context.Context, metric, focusLabel, topN, date string) (TSDBWithMetricResult, error) {
	u := h.client.URL(epTSDBWithMetric, nil)
	q := u.Query()

	if focusLabel != "" {
		q.Set("focusLabel", focusLabel)
	}
	...
	req, err := http.NewRequest(http.MethodGet, u.String(), nil)
	if err != nil {
		return TSDBWithMetricResult{}, err
	}
	...
	var res TSDBWithMetricResult
	err = json.Unmarshal(body, &res)
	return res, err
}

func (h *httpAPI) TopQueries(ctx context.Context, topN, topNMaxLifeTime string) (TopQueriesResult, error) {
	u := h.client.URL(epTopQueries, nil)
	q := u.Query()

	q.Set("topN", topN)
	q.Set("maxLifetime", topNMaxLifeTime)
	...
	var res TopQueriesResult
	err = json.Unmarshal(body, &res)
	return res, err
}

func (h *httpAPI) TSDB(ctx context.Context, topN, date string) (TSDBResult, error) {
